...
; Robust Inno Setup Installer Script v1.2.0
; Features: Improved process handling, better uninstall, portable support
...
; Output settings
OutputDir=.\output
OutputBaseFilename=MBAS_Setup_v1.2.0_Robust
...
WelcomeLabel2=This will install {#MyAppName} on your computer.%n%nRecommended installation path: C:\MBAS (no admin required)%nProgram Files requires admin permissions for every operation.%n%nIt is recommended that you close all other applications before continuing.
...
Filename: "cmd.exe"; Parameters: "/C taskkill /F /IM python.exe /T & taskkill /F /IM pythonw.exe /T & taskkill /F /IM node.exe /T"; RunOnceId: "StopMBAS_Force"; Flags: runhidden waituntilterminated

; Wait for processes to fully terminate
Filename: "cmd.exe"; Parameters: "/C timeout /t 3 /nobreak"; RunOnceId: "WaitForProcesses"; Flags: runhidden waituntilterminated

; Release file handles using PowerShell
Filename: "powershell.exe"; Parameters: "-ExecutionPolicy Bypass -Command ""Get-Process | Where-Object {{ $_.Path -like '*{app}*' }} | Stop-Process -Force"""; RunOnceId: "ReleaseHandles"; Flags: runhidden skipifdoesntexist
...
// Forcefully kill all MBAS processes
...
begin
  ...
  // Kill any process using MBAS ports
  Exec('cmd.exe', '/C for /f "tokens=5" %a in (''netstat -ano ^| findstr ":8000" 2^>nul'') do taskkill /F /PID %a', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
  Exec('cmd.exe', '/C for /f "tokens=5" %a in (''netstat -ano ^| findstr ":3000" 2^>nul'') do taskkill /F /PID %a', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
  Exec('cmd.exe', '/C for /f "tokens=5" %a in (''netstat -ano ^| findstr ":5173" 2^>nul'') do taskkill /F /PID %a', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
...
begin
  ...
  begin
    ...
    begin
      ...
      Exec('powershell.exe',
           '-ExecutionPolicy Bypass -Command "Get-Process | Where-Object { $_.Path -like ''*' + AppDir + '*'' } | Stop-Process -Force"',
