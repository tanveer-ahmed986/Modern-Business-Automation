...
class SaleBase(SQLModel):
    invoice_number: str = Field(unique=True, index=True)  # CRITICAL: Unique invoice tracking
...
class SaleItemBase(SQLModel):
    ...
    unit_price: Decimal = Field(default=0.0) # Snapshot of selling price at time of sale
    cost_price: Decimal = Field(default=0.0)  # CRITICAL: Snapshot of cost price for accurate profit calculation
