...
class SalesForecaster:
    ...
    def predict_next_n_days(self, n_days: int = 30) -> List[dict]:
        ...
        """
        if not _HAS_ML_DEPS or not self.trained:
            return []

        # Get the last date from training data
        last_training_date = pd.to_datetime(self.model.feature_names_in_[0] if hasattr(self.model, 'feature_names_in_') else '1970-01-01')
