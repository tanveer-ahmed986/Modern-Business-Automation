...
echo [Step 4/6] Installing dependencies (this may take 2-4 minutes)...
echo.
echo     [*] Installing core backend packages with locked versions...
...
echo     [*] This prevents first-startup errors...
python -m pip install pystray Pillow psutil requests --quiet --disable-pip-version-check
if errorlevel 1 (
    echo [WARN] Failed to install some tray dependencies, will retry on first start
) else (
    echo [OK] System tray dependencies installed
)
echo.

echo [Step 6/6] Initializing database and creating shortcuts...
cd /d "%~dp0backend"
